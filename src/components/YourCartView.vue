            ...
            <div v-else class="flex flex-col m-8 " >
                    <div  v-for="item in card"     :key="item.name" >
                        <div v-if="item.contity >= 1" class="flex justify-between">
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="10" height="2"  viewBox="0 0 10 10" class="w-6 h-6 border border-solid border-accent rounded-full p-1  fill-accent hover:border-2 my-auto" @click="remove(item)"
                        ...
                        <div  class="bg-primary rounded-full text-center text-white py-6 mb-4 cursor-pointer">
                            Confirm Order
                        </div>
            </div>
